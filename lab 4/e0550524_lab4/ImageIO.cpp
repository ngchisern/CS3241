...
int ImageIO::SaveImageToFilePNG(const std::string &filename, const uchar *imageData,
                                int imageWidth, int imageHeight, int numComponents)
{ 
    stbi_flip_vertically_on_write(true);

    int write_status = stbi_write_png(filename.data(), imageWidth, imageHeight, numComponents, imageData, 0);
...
int ImageIO::SaveImageToFileJPEG(const std::string &filename, const uchar *imageData,
                        int imageWidth, int imageHeight, int numComponents, int quality)
{
    stbi_flip_vertically_on_write(true);

    int write_status = stbi_write_jpg(filename.data(), imageWidth, imageHeight, numComponents, imageData, quality);
